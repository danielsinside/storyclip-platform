...
serve(async (req) => {
  ...
  try {
    ...
    // 1. Render API (NEW v2)
    if ((action || "").toLowerCase() === "render-create" || (req.method === "POST" && url.pathname.includes("/render"))) {
    ...
    // 4. Direct process-video (RECOMMENDED) - Try multiple endpoints as fallback
      ...
      // Punto de log puntual solicitado (sin streaming)
      try {
        console.log({
          evt: "proxy_process_video_req",
          clips: (payload as any)?.clips?.length,
          clip0Effects: (payload as any)?.clips?.[0]?.effects,
        });
      } catch (_) {
        // noop
      }
