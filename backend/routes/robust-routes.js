...
const OUTPUT_UPLOAD_DIR = '/srv/storyclip/outputs/uploads';

// Configurar multer con storage estable
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    // OPTIMIZACIÓN: Escribir directo al directorio final para evitar fs.move()
    // Esto elimina la doble escritura en disco y reduce latencia post-upload
    fs.mkdirSync(OUTPUT_UPLOAD_DIR, { recursive: true });
    cb(null, OUTPUT_UPLOAD_DIR);
  },
  filename: (req, file, cb) => {
    ...
    // Siempre usar .mp4 como extensión final (garantizado)
    const safe = `${uploadId}.mp4`;
...
router.post('/videos/upload', uploadMulter.single('file'), async (req, res) => {
  try {
    ...
    const outputPath = path.join(OUTPUT_UPLOAD_DIR, fileName);
    
    // OPTIMIZACIÓN: Detectar si multer ya escribió en el lugar correcto
    // Si req.file.path ya está en OUTPUT_UPLOAD_DIR, no necesitamos mover
    const needsMove = req.file?.path && !req.file.path.startsWith(OUTPUT_UPLOAD_DIR);
    
    let finalPath = outputPath; // Por defecto, usar el path esperado

    if (needsMove) {
      // Fallback: Si por alguna razón multer escribió en otro lugar, moverlo
      logger.info(`[UPLOAD] Moved from tmp to final location: ${req.file.path} -> ${outputPath}`);
      await fs.ensureDir(OUTPUT_UPLOAD_DIR);
      await fs.move(req.file.path, outputPath);
      finalPath = outputPath;
    } else {
      // OPTIMIZACIÓN: Archivo ya está en el lugar correcto (direct-write)
      finalPath = req.file.path;
      logger.info(`[UPLOAD] Direct write to final location: ${finalPath} (${req.file?.size} bytes)`);
    }
    ...
    logger.info(`[UPLOAD] Upload registered: ${meta.uploadId} -> ${finalPath} (${req.file?.size} bytes)`);

    // Verificar que el archivo existe
    if (!await fs.pathExists(finalPath)) {
      logger.error(`[UPLOAD] File not found after write: ${finalPath}`);
...
router.post('/process-video', async (req, res) => {
  try {
    ...
    // Generar jobId único SIEMPRE para permitir reprocesamiento
    // No reutilizar uploadId como jobId para evitar conflictos UNIQUE constraint
    let jobId = req.body.jobId;
    if (!jobId) {
      jobId = createJobId();  // Siempre generar nuevo ID único
    }
      ...
      try {
        ...
        if (result.success) {
          ...
          updateJob(jobId, {
            status: 'done',
            progress: 100,
            outputUrl: result.outputUrl || `${CDN_BASE}/uploads/${jobId}.mp4`,
...
router.get('/jobs/:jobId/status', async (req, res) => {
  try {
    ...
    if (job) {
      ...
      // IMPORTANTE: Solo devolver artifacts si el job está completado (done)
      // Durante el procesamiento, NO devolver artifacts para evitar 404s
      ...
      // Si está procesando, NO devolver artifacts (evita 404s en el frontend)
      ...
      return res.json({
        ...
        outputs: job.status === 'done' ? (job.outputs || []) : [], // Solo outputs si está done
        errorMessage: job.errorMessage,
        message: job.message,
        metadata: job.metadata,
        result: {
          artifacts: artifacts // Vacío durante procesamiento, lleno cuando done
        }
      });
    }
    ...
    // Si no está en memoria, intentar buscar en base de datos
    try {
      const robustProcessing = require('../services/robust-processing.service');
      const dbStatus = await robustProcessing.getJobStatus(jobId);

      if (dbStatus && dbStatus.status !== 'pending') {
        // Construir artifacts desde el resultado de la DB
        let artifacts = [];
        if (dbStatus.result && dbStatus.result.artifacts) {
          artifacts = dbStatus.result.artifacts;
        }
        ...
        return res.json({
          ...
          outputUrl: dbStatus.outputs?.[0] || null,
          outputs: dbStatus.outputs || [],
      ...
      logger.warn(`Failed to get job status from DB: ${e.message}`);
    }
...
router.get('/v1/jobs/:jobId/status', async (req, res) => {
  try {
    ...
    if (job) {
      ...
      // Construir outputs array (URLs simples) - solo si está done
      const outputUrls = (job.status === 'done' || job.status === 'completed') ?
        (job.outputs ?
 ...
 * Generar metadata y presets para el video
 */
router.post('/generate-preset', async (req, res) => {
  try {
    ...
    if (!videoUrl && !uploadId) {
      return res.status(400).json({
        success: false,
        error: 'videoUrl or uploadId is required'
      });
    }

    // Generar preset básico basado en el video
    const preset = {
      aspectRatio: '9:16',
      clipDuration: 5,
      maxClips: 10,
    ...
    logger.error('Generate preset error:', error);
    return res.status(500).json({
      success: false,
